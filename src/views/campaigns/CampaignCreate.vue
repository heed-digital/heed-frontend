...
<CCard style="padding: 20px"> 
  ...
  <div>
    <CForm>
        ...
        <button class="btn btn-success text-center float-right" v-on:click="onClickCreateCampaign" style="margin-left: 10px; margin-top: 20px; margin-bottom:20px"><b>Create campaign</b></button>
    </CForm>
  </div>
  </CCard>
</template>
...
export default {
    ...
    data () {
        return {
            ...
            modal : {
                ...
                content: 'Please fill in all the required information: '
            },
    ...
    beforeCreate () {
        ...
        console.log('beforeCreate campaign', this.$store.campaign);
    ...
    methods: {
        ...
        get_campaign_id () {
            if (this._campaign && this._campaign.id) return this._campaign.id; // only here in campaign create
        ...
        GET_campaign_users (callback) {
            var campaign_id = this.get_campaign_id();
            if (_.isUndefined(campaign_id)) {
                console.error('undefined campaign_id');
                return callback([]);
            }
        ...
        onClickCreateCampaign (val) {

            // ensure campaign data is valid
            var missing = this.ensure_valid_campaign();
            if (missing) {
                return this.mark_invalid_fields(missing);
            }

            // save campaign data
            this.push_new_campaign(function () {
        ...
        mark_invalid_fields (missing) {
            this.modal.title = 'Missing information'
            this.modal.content = 'Please fill in the required information: \n' + missing.join(', ') + '.';
            this.warningModal = true;
        },

        ensure_valid_campaign () {
            var missing = [];

            // ensure active
            if (_.isNil(this.$store.campaign.active)) {
                this.$store.campaign.active = false;
            }

            // ensure name
            if (_.isNil(this.$store.campaign.name) || !_.size(this.$store.campaign.name)) {
                missing.push('name');
            }

            // ensure frequency
            if (_.isNil(this.$store.campaign.frequency)) {
                missing.push('frequency');
            }

            // ensure start date
            if (_.isNil(this.$store.campaign.start_date)) {
                missing.push('start_date');
            }

            if (!_.size(missing)) return false;
            return missing;

        },

        push_new_campaign (callback) {

            // set endpoint
            this.$http.post('/campaigns', this.$store.campaign, getHeaders())
            .then(function (response) {
                console.log('[push_new_campaign]: response ', response);
                this._campaign = response.data;
                return callback();
            }.bind(this))
            .catch(function (err) {
                console.log('[push_new_campaign]: axios post error: ', err, err.response);
                return callback();
            });
        },
        push_campaign_users () {
        ...
        add_campaign_users (add) {
            ...
            // set endpoint
            var campaign_id = this.get_campaign_id();
            if (!campaign_id) return console.error('no campaign id?!');
        ...
        rem_campaign_users (rem) {
            ...
            var campaign_id = this.get_campaign_id();
            if (!campaign_id) return console.error('no campaign id?!');
            var endpoint = '/campaigns/' + campaign_id + '/users/remove';
